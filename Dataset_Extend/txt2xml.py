...
# 源文件夹和目标文件夹路径
source_folder = 'test_label_enhance'      # 替换为存放txt标签的文件夹路径
target_folder = 'test_label_txt2xml'      # 替换为保存xml文件的文件夹路径
image_folder = 'D:\\faster-rcnn-pytorch-master\\VOCdevkit\\VOC2007\\JPEGImages'    # 替换为存放图片的文件夹路径
...
for txt_file in os.listdir(source_folder):
    if txt_file.endswith('.txt'):
        ...
        image_file_name = txt_file.replace('.txt', '.jpg')  # 图像文件是jpeg格式
